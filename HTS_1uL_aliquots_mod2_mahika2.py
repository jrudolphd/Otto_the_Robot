...
metadata = {
    'protocolName': 'High Throuput Screen for PARP ± HPF1 (Mahika edits)',
    'requires':'10mL buffer in boat well 1, 5mL PARP1, 5mL PARP1+HPF1',
...
def run(protocol_context):
    ...
    p300m.flow_rate.aspirate = 15
    p300m.flow_rate.dispense = 15
...
class AssayPrep:
    ...
    def fill_plate_with_buffer(self, rangestart, rangeend, evenodd):
        # aspirate 240 µL from the buffer well (only once)
        self.p300m.aspirate(250, self.buffer)

        # distribute 20 µL from the aspirated 240 µL to each well in the selected row (even or odd)
        for row in range(rangestart, rangeend):
    ...
    def fill_plate_with_enzyme_1 (self, evenodd, rangestart, rangeend):
        self.p300m.aspirate(130, self.enzyme_1)
    ...
    def fill_plate_with_enzyme_2 (self, evenodd, rangestart, rangeend):
        self.p300m.aspirate(130, self.enzyme_2)
